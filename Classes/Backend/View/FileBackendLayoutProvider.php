...
namespace WEBcoast\FileBackendLayouts\Backend\View;
...
class FileBackendLayoutProvider implements BackendLayout\DataProviderInterface
{
            ...
            foreach ($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['file_backend_layouts']['BackendLayouts'] as $extensionKey => $configuration) {
                ...
                if (is_array($fileNames)) {
                    foreach ($fileNames as $fileName) {
                        ...
                        if (preg_match('/^(.*)\.(?:typoscript|tsconfig)$/', $fileName, $matches)) {
                            $identifier = $extensionKey . '-' . $matches[1];
    ...
    public function getBackendLayout($identifier, $pageId)
    {
        ...
        $configuration = $GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['file_backend_layouts']['BackendLayouts'][$extensionKey];
        list ($path, $languageFile) = $configuration;
        // set default path, if no path is set
        if (empty($path)) {
            $path = '/Configuration/BackendLayouts/';
        ...
        foreach (['typoscript', 'tsconfig'] as $allowedFileExtension) {
            $fileName = $identifier . '.' . $allowedFileExtension;
